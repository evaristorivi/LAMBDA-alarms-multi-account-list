...
def lambda_handler(event, context):
        try:
                ...
                a=[]
                for nuevofull in fullresponse:
                        a.append(nuevofull["MetricAlarms"])
                save_obj(a,FICHERO_DATOS)
                # Subimos el fichero a s3
                s3.Bucket(BUCKET).upload_file('/tmp/' + FICHERO_DATOS, FICHERO_DATOS)
                        
        except:
                # Control de errores de la Lambda::
                send_email("[LAMBDA ALARMS-MULTI-ACCOUNT-LIST - FAILURE]", "La Lambda ALARMS-MULTI-ACCOUNT-LIST se ha lanzado con errores.")